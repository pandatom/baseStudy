...
![image-20210917160105124](/Users/mac/Library/Application Support/typora-user-images/image-20210917160105124.png)
...
如果http也配置了一样的server；那么四层的处理优先级大于七层；会优先执行四层的server。
...
```
	...
	表示.js文件时候，就不缓存文件，条件是$nocache，放到proxy_no_cache和proxy_cache_bypass中，只要值不为0，就不走缓存。
	if ($request_uri ~ /.*\.js$){
	       set $nocache 1;
	    }
...
建议使用不缓存的时候 proxy_no_cache和proxy_cache_bypass都配置上。
